...
def plotBipartiteGraph(
    adjacencyMatrix, donorLabels=None, agencyLabels=None, figureSize=(15, 11)
):
    ...
    nx.draw_networkx_edges(
        bipartiteGraph, nodePositions, edge_color="gray", width=2, alpha=0.6
    )
    ...
    # customize the plot
    plt.title(
        "Bipartite Graph of Donors to Agencies", fontsize=16, fontweight="bold", pad=20
    )
    plt.text(
        0,
        max(numDonors, numAgencies) + 0.5,
        "Food Donors",
        fontsize=14,
        fontweight="bold",
        ha="center",
...
def plotAllocationGraph(
    ...
    # customize the plot
    plt.title(
        "Food Allocation Results: Actual Donor-Agency Transfers",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def plotComparisonGraphs(
    ...
    ax2.set_title(
        "Allocation Results\n(Actual Food Transfers)", fontsize=12, fontweight="bold"
    )
